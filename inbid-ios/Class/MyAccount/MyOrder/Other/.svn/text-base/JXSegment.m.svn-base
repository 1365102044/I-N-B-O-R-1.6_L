...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _divideLineView.backgroundColor = UIColorFromRGB(0xe5e5e5);
        [_scrollView addSubview:_divideLineView];
        
        _divideView  = [[UIView alloc] init];
        _divideView.backgroundColor = [UIColor mainThemColor];
        ...
        [YXNotificationTool addObserver:self selector:@selector(formeVCscrollIndex:) name:@"formeVCscrollIndex" object:nil];
    }
...
- (void)updateChannels:(NSArray*)array{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        [button setTitleColor:UIColorFromRGB(0x787575) forState:UIControlStateNormal];
        [button setTitleColor:[UIColor mainThemColor] forState:UIControlStateSelected];
