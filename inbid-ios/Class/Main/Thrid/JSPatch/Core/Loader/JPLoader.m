...
{
    context[@"include"] = ^(NSString *filePath) {
        NSArray *component = [filePath componentsSeparatedByString:@"main.js"];
...
//    NSURL *downloadURL = [NSURL URLWithString:[STYTEM_THIRD_JSPATCH stringByAppendingString:downloadKey]];
//    NSURLRequest *request = [NSURLRequest requestWithURL:downloadURL cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:20.0];
    ...
    //** Replace - Start */
    NSURL *downloadURL = [NSURL URLWithString: STYTEM_THIRD_JSPATCH];
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:downloadURL];
    request.cachePolicy = NSURLRequestReloadIgnoringLocalCacheData;
    request.timeoutInterval = 20.0;
    [request setHTTPMethod:@"POST"];
    NSMutableData *postBody = [NSMutableData data];
    [postBody appendData:[[NSString stringWithFormat:@"appType=%@&hotVersion=%@.%zd",@"1",appVersion, version] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        NSDictionary *responseHeader = newResponse.allHeaderFields;
        
        if (!error && [responseHeader[@"Status"] isEqualToString:@"1"]) {
            ...
            if (!isFailed) {
                ...
                //** Original - Start */
//                NSData *md5Data = [RSA decryptData:[NSData dataWithContentsOfFile:keyFilePath] publicKey:STYTEM_THIRD_JSPATCH_PUBLICKEY];
...
//                NSString *md5 = [self fileMD5:scriptZipFilePath];
                ...
                NSString *decryptMD5 = [RSAEncryptor decryptString:[[NSString alloc] initWithData:[NSData dataWithContentsOfFile:keyFilePath] encoding:NSUTF8StringEncoding] privateKeyWithContentsOfFile:private_key_path password:@"123456"];
                NSString *md5 = [self fileMD5:scriptZipFilePath];
                //** Replace - End */
            ...
            if (![responseHeader[@"Status"] isEqualToString:@"1"]) {
                callback([NSError errorWithDomain:[NSString stringWithFormat:@"org.jspatch\n%@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]] code:JPUpdateErrorSeverReturnFailed userInfo:nil]);
            }else{
                if (JPLogger) JPLogger([NSString stringWithFormat:@"JSPatch: request error %@", error]);
                YXLog(@"JSPatch: request error %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
